...
# ── Sentinel prefixes ──────────────────────────────────────────────────────────
_QUOTA_PREFIX = "QUOTA_EXHAUSTED:"
_ERROR_PREFIX = "GEMINI_ERROR:"

def call_gemini(messages: list, temperature: float = 0.4, json_mode: bool = False) -> str:
    system_prompt = ""
    contents = []
    ...
    # To this:
    if not contents:
        # System-only call with no user turn — add a minimal trigger
    ...
    try:
        ...
        # Empty text — check why
        try:
            reason = str(response.candidates[0].finish_reason) if response.candidates else "NO_CANDIDATES"
        except Exception:
            reason = "UNKNOWN"
        print(f"Gemini empty response, finish_reason={reason}")
        return f"{_ERROR_PREFIX}Empty response from Gemini (reason: {reason})"

    except Exception as e:
        error_str = str(e)
        is_quota = any(kw in error_str for kw in ["429", "quota", "RESOURCE_EXHAUSTED", "exceeded"])
        if is_quota:
            print(f"Gemini quota exhausted: {error_str[:150]}")
            return f"{_QUOTA_PREFIX}Quota exhausted. Please try again in a few minutes."
        print(f"Gemini error: {error_str[:200]}")
        return f"{_ERROR_PREFIX}{error_str[:200]}"


def _is_quota(text: str) -> bool:
    return text.startswith(_QUOTA_PREFIX)

def _is_error(text: str) -> bool:
    return text.startswith(_ERROR_PREFIX)

def _is_bad(text: str) -> bool:
    """True when the response is empty, a quota failure, or a hard error."""
    return not text or _is_quota(text) or _is_error(text)

def _user_message(text: str) -> str:
    """Convert an internal sentinel into a human-readable message."""
    if _is_quota(text):
        return "AI quota exhausted. Please try again in a few minutes."
    if _is_error(text):
        return f"AI error: {text[len(_ERROR_PREFIX):]}"
...
def analyze_with_llm(resume_text, jd_text):
    ...
    if _is_bad(raw_response):
        print(f"analyze_with_llm: {_user_message(raw_response)}")
        return {"name": "Candidate", "skills_found": [], "skills_missing": [], "reasoning": _user_message(raw_response)}

    clean_json = re.sub(r'```(?:json)?\s*|\s*```', '', raw_response, flags=re.MULTILINE).strip()
    try:
        parsed = json.loads(clean_json)
        if isinstance(parsed, dict) and "skills_missing" not in parsed:
            for v in parsed.values():
                if isinstance(v, dict) and "skills_missing" in v:
                    return v
        return parsed
    except Exception as e:
        print(f"LLM parse error: {e}\nRaw: {raw_response[:300]}")
        return {"name": "Candidate", "skills_found": [], "skills_missing": [], "reasoning": "Parse error"}
...
async def generate_roadmap(user_id: Optional[str] = None):
    try:
        ...
        raw_roadmap = call_gemini(
            messages=[{"role": "user", "content": prompt}],
            temperature=0.5,
            json_mode=True,
        )

        if _is_bad(raw_roadmap):
            return {"error": _user_message(raw_roadmap)}

        clean_json = re.sub(r'```(?:json)?\s*|\s*```', '', raw_roadmap, flags=re.MULTILINE).strip()
        parsed_roadmap = json.loads(clean_json)
        if isinstance(parsed_roadmap, dict):
            for v in parsed_roadmap.values():
                if isinstance(v, list):
                    parsed_roadmap = v
                    break
        roadmap_data = parsed_roadmap if isinstance(parsed_roadmap, list) else []
...
async def chat_with_ai(request: ChatRequest):
    try:
        ...
        if q_num > 10:
            ...
            if _is_bad(raw_eval):
                return {
                    "text": _user_message(raw_eval),
                    "is_complete": True,
                    "passed": False,
                }

            content = re.sub(r'^```json\s*|```$', '', raw_eval, flags=re.MULTILINE).strip()
            try:
                result = json.loads(content)
            except Exception:
                return {"text": f"Parse error in evaluation: {content[:100]}", "is_complete": True, "passed": False}
        ...
        for msg in request.messages:
         ...
         # ── ADD THIS BLOCK ──────────────────────────────────────────────
        # On the very first call messages is empty — Gemini needs at least
        # one user turn to generate a response.
        has_user_turn = any(m["role"] == "user" for m in messages)
        if not has_user_turn:
            messages.append({
                "role": "user",
                "content": "Begin the examination. Ask me the first question now."
            })
        # ───────────────────────────────────────────────────────────────
        ...
        if _is_bad(response_text):
            return {
                "text": _user_message(response_text),
                "is_complete": False,
                "q_num": q_num,
        ...
        return {"text": f"Server error: {str(e)}", "is_complete": False}
...
async def evolve_resume(
    ...
    try:
        ...
        if _is_bad(raw):
            return {"error": _user_message(raw)}
...
async def upload_resume(
    ...
    try:
        ...
        if _is_bad(raw):
            return {"status": "error", "message": _user_message(raw)}
...
async def search_jobs(session_id: Optional[str] = None):
    ...
    if _is_bad(search_query_raw):
        raise HTTPException(status_code=500, detail=_user_message(search_query_raw))

    search_query = search_query_raw.strip('"').strip("'")
...
async def match_job(request: MatchRequest):
    ...
    try:
        ...
        if _is_bad(raw_match):
            return {"error": _user_message(raw_match)}
...
"""
        ...
        if _is_bad(raw_intro):
            return {"error": _user_message(raw_intro)}

        result = json.loads(raw_intro)
...
"""
        ...
        raw_regen = call_gemini(
            messages=[{"role": "user", "content": prompt}],
            temperature=0.7,
            json_mode=True,
        )

        if _is_bad(raw_regen):
            return {"error": _user_message(raw_regen)}

        return json.loads(raw_regen)
...
async def career_chatbot(request: ChatbotRequest):
    try:
        system_prompt = (
            "You are CareerAI, an expert AI career advisor for engineering students in India. "
            "You help with: resume tips, interview prep, skill roadmaps, higher studies (MS/MBA/MTech), "
            "job searching, salary negotiation, and career switching. "
            "Keep answers concise, practical, and India-specific where relevant. "
            "Use bullet points for lists. Never fabricate company names or statistics."
        )

        if request.user_id:
            user = load_user(request.user_id)
            if user:
                gaps    = ", ".join(user.get("gaps", [])) or "none identified"
                skills  = ", ".join(user.get("verified_skills", [])) or "none yet"
                system_prompt += (
                    f"\n\nCANDIDATE PROFILE:"
                    f"\n- Name: {user.get('name', 'Student')}"
                    f"\n- Verified Skills: {skills}"
                    f"\n- Skill Gaps: {gaps}"
                    f"\n- Resume Summary: {user.get('resume_summary', 'Not uploaded yet')[:300]}"
                    f"\nPersonalise every response using this context."
                )
        ...
        reply = call_gemini(messages=messages, temperature=0.5)

        if _is_bad(reply):
            return {"reply": _user_message(reply)}
...
@app.post("/api/chatbot-vision")
async def career_chatbot_vision(
    message: str = Form(...),
    user_id: Optional[str] = Form(None),
    ...
    try:
        system_text = (
            ...
            "If an image is provided (marksheet, certificate, resume), extract all relevant information "
            "from it and use it to give highly specific career advice. "
            "For marksheets: identify CGPA/percentage, subjects, strong/weak areas. "
            "For certificates: identify skills and suggest how to leverage them. "
            "For resumes: give targeted improvement advice."
        )
        ...
        if image:
            ...
            content_parts = [
                types.Part(inline_data=types.Blob(mime_type=image.content_type, data=b64)),
...
## Resume evolution and recommendation enhancements